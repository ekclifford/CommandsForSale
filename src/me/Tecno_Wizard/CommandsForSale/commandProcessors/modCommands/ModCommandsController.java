...
public class ModCommandsController implements CommandExecutor 
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		if(args.length > 0)
		{
			...
			{
			case "reload": ReloadConfigExecutor rce = new ReloadConfigExecutor(main);
				rce.runCommand(sender, args);
				break;
			case "addcmd": AddCmdExecutor ace = new AddCmdExecutor(main);
				ace.runCommand(sender, args);
				break;
			case "updatestatus": UpdatePluginExecutor uce = new UpdatePluginExecutor(main);
				uce.runCommand(sender);
				break;
			case "addalias": AddCmdAliasExecutor acae = new AddCmdAliasExecutor(main);
				acae.runCommand(sender, args);
				break;
			default: sender.sendMessage(String.format("[%s] That is not a valid subcommand. <reload || addcmd || addalias || updatestatus>", pluginPrefix));
				break;
			}
		}
		else
